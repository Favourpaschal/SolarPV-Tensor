      ...
      <AnimatePresence mode="wait">
        ...
        {step === 0 && (
          ...
          >
            ...
            >
              <option value="abuja">Abuja (5.5 PSH)</option>
              <option value="lagos">Lagos (4.8 PSH)</option>
              <option value="kano">Kano (6.2 PSH)</option>
          ...
          >
              ...
              {[
                {
                  ...
                  sub: `Peak: ${result.load.peak_load_w}W`,
                },
                {
                  label: 'Solar array',
            ...
            {result.inverter_sizing && (
              <div style={{
                ...
                <div style={{ fontSize: 12, fontWeight: 600, color: '#27500A', marginBottom: 6 }}>
                  ⚡ Inverter sizing
                </div>
                <div style={{ fontSize: 13, color: '#333' }}>
                  Minimum required:{' '}
                  <strong>{result.inverter_sizing.min_rating_w}W</strong>
                  {' '}(based on {result.load.peak_load_w}W peak load × {result.inverter_sizing.safety_margin} safety margin)
                </div>
                ...
                {result.top_inverters && result.top_inverters.length === 0 && (
                  <div style={{ marginTop: 6, fontSize: 12, color: '#666' }}>
                    No inverter match found in database — seed inverter data in Supabase.
                  </div>
                )}
              </div>
            )}
            ...
            {result.top_panels && result.top_panels[0] && (
              <div style={{
                ...
                <div style={{ fontSize: 13, color: '#333' }}>
                  <strong>
                    {result.top_panels[0].manufacturer} {result.top_panels[0].model}
                  </strong>
              ...
              <div style={{
                ...
                {Object.values(result.wire_recommendations).map((w: any) => (
                  ...
                  >
                    <span>{w.run}</span>
                    <span>
                      <strong>{w.awg}</strong> ({w.mm2}mm²) —{' '}
                      {w.voltage_drop_pct}% drop
                    </span>
                  </div>
                ))}
              </div>
              ...
              <div style={{
                ...
                <ul style={{ margin: 0, paddingLeft: 18 }}>
                  {result.tool_checklist.map((t: any) => (
                    <li key={t.tool} style={{ fontSize: 12, color: '#444', marginBottom: 4 }}>
            ...
            {/* Simulation results + report exporter */}
            {simResult && (
              <>
